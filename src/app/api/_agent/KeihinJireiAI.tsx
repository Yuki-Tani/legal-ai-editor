...
async function doRequestCommentKeihin(
  ...
  let systemMessage = `法律文章についてのアイデアと要件、それによって生成された文章、関連する景品表示法処分事例、ユーザとのやりとりが与えられます。以下の景品表示法処分事例からユーザーの文章と関連するものを１つ引用して500文字以内で文章についての修正提案コメントを考えてください。回答には「事例の処分日時、サービス、処分内容、表示と実際、違反分類、罰則」などの処分事例を詳細に要約した散文文章を含むコメントのみを返信してください。「参考文献は消費者庁の公式サイトです」、と文末に記載してください。
  アイデアと要件；${coreIdea}\n\nユーザーの文章；${selectedText}\n\n景品表示法処分事例：${searchResults}`;
  if (searchText == coreIdea) {
    systemMessage = `法律文章についてのアイデアと要件、関連するかもしれない景品表示法処分事例、これまでの会話が与えられます。これまでの会話の流れを汲み取りつつ、以下の景品表示法処分事例からユーザーの文章と関連するもの１つ以上引用して、500文字以内で詳細で具体的で法律文書作成のために建設的なコメントを考えてください。回答には「事例の処分日時、サービス、処分内容、表示と実際、違反分類、罰則」などの処分事例を詳細に要約した散文文章を含むコメントのみを返信してください。「参考文献は消費者庁の公式サイトです」、と文末に記載してください。
  アイデアと要件；${coreIdea}\n\n景品表示法処分事例：${searchResults}`;
  }
