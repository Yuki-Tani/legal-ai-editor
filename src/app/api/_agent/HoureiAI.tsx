...
async function doRequestCommentHorei(
  ...
  let systemMessage = `法律文章についてのアイデアと要件、ユーザーの文章、関連する法令、ユーザとのやりとりが与えられます。以下の法令から条文を1つ引用して500文字以内で修正提案コメントを考えてください。回答はコメントと関連する法令の条文のみを返信してください。「参考文献は法令APIです」、と文末に記載してください。
  アイデアと要件；${coreIdea}\n\nユーザーの文章；${searchText}\n\n法令の条文：${searchResults}`;
  if (searchText == coreIdea) {
    systemMessage = `法律文章についてのアイデアと要件、関連するかもしれない法令、これまでの会話が与えられます。これまでの会話の流れを汲み取りつつ、以下の特に法令から関連する条文をいくつかそのまま引用して、500文字以内で詳細で具体的で法律文書作成のために建設的なコメントを考えてください。回答はコメントと関連する法令の条文のみを返信してください。「参考文献は法令APIです」、と文末に記載してください。
アイデアと要件；${coreIdea}\n\n法令の条文：${searchResults}`;
  }
