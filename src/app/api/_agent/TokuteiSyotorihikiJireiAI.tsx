...
async function doRequestCommentTokutei(
  ...
  let systemMessage = `法律文章についてのアイデアと要件、それによって生成された文章、関連する特定商取引法違反執行事例、ユーザとのやりとりが与えられます。以下の特定商取引法違反執行事例からユーザーの文章と関連するものを１つ引用して500文字以内で文章についての修正提案コメントを考えてください。回答には引用した特定商取引法違反執行事例を具体的・詳細に要約した文章を含むコメントのみを返信してください。「参考文献は消費者庁の公式サイトです」、と文末に記載してください。
  アイデアと要件；${coreIdea}\n\n文章；${selectedText}\n\n特定商取引法違反執行事例：${searchResults}`;
  if (searchText == coreIdea) {
    systemMessage = `法律文章についてのアイデアと要件、関連する特定商取引法違反執行事例、これまでの会話が与えられます。これまでの会話の流れを汲み取りつつ、以下の特定商取引法違反執行事例からユーザーの文章と関連するものを１つ以上引用して、500文字以内で詳細で具体的で法律文書作成のために建設的なコメントを考えてください。回答には引用した特定商取引法違反執行事例を具体的・詳細に要約した文章を含むコメントのみを返信してください。「参考文献は消費者庁の公式サイトです」、と文末に記載してください。
  アイデアと要件；${coreIdea}\n\n特定商取引法違反執行事例：${searchResults}`;
  }
