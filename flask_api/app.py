...
def analyze_document():
    ...
    if not document:
        return jsonify({"error": "テキストが提供されていません。"}), 400
    prompt = f"""
    以下の法律に関する文書から、トピックを1つ以上抽出してください。なるべく法律や行政とは関係のないトピックを選んでください。
    さらに、そのトピックに関する最も最新のニュース、関連企業、関連商品、専門情報をウェブ検索して、非常に詳しく要約してください。回答は要約と参考文献のみにしてください。他の文字を含めないでください。
    文書: {document}
    """
